...
namespace WebSite.Common
{
    ...
    public class ImageUpload
    {
        ...
        private string _FileType = "jpg;gif;bmp;png;jpeg";//所支持的上传类型用";"隔开 
        ...
        private int _ThumbStyle = 0;//输出缩略图片样式，0：按宽、高中最大的来等比缩略， 1：按宽来等比缩略，2：按高来等比缩略
        private int _MidStyle = 0;//输出缩略图片样式，0：按宽、高中最大的来等比缩略， 1：按宽来等比缩略，2：按高来等比缩略
        private string _OutFileName = "";//输出文件名。 
        private string _OutThumbFileName = "";//输出小缩略文件名。
        private string _OutMidFileName = "";//输出中缩略文件名。
        private bool _IsCreateThumbImg = false;//是否生成小缩略图。 
        private bool _IsCreateMidImg = false;//是否生成中缩略图。 
        private bool _IsThumbOK = false;//是否有小缩略图生成.
        private bool _IsMidOK = false;//是否有中缩略图生成.
        private int _Height = 0;//获取上传图片的高度 
        private int _Width = 0;//获取上传图片的宽度 
        private int _tHeight = 100;//设置生成小缩略图的高度 
        private int _tWidth = 100;//设置生成小缩略图的宽度
        private int _mHeight = 150;//设置生成中缩略图的高度 
        private int _mWidth = 150;//设置生成中缩略图的宽度
        ...
        /// <summary>
        /// 输出缩略图片样式，0：按宽、高中最大的来等比缩略， 1：按宽来等比缩略，2：按高来等比缩略，3：按宽、高中最小的来缩略其他裁剪
        /// </summary>
        ...
        public string OutThumbFileName
        {
            ...
            set { _OutThumbFileName = value; }
        }

        /// <summary>
        /// 输出的大缩略图文件名
        /// </summary>
        public string OutMidFileName
        {
            get { return _OutMidFileName; }
            set { _OutMidFileName = value; }
        }

        /// <summary>
        /// 是否有小缩略图生成.
        /// </summary>
        public bool IsThumbOK
        {
            get { return _IsThumbOK; }
        }
        ...
        /// 是否生成小缩略图
        /// </summary>
        public bool IsCreateThumbImg
        {
            get { return _IsCreateThumbImg; }
            set { _IsCreateThumbImg = value; }
        }

        /// <summary>
        /// 是否生成中缩略图
        /// </summary>
        public bool IsCreateMidImg
        {
            get { return _IsCreateMidImg; }
            set { _IsCreateMidImg = value; }
        }
        ...
        public void Upload()
        {
            ...
            try
            {
                ...
                if (_IsCreateThumbImg)
                {
                    ...
                    if (_Width <= _tWidth && _Height <= _tHeight)
                    {
                        _tWidth = _Width;
                        _tHeight = _Height;
                    }
                    else
                    {
                        ...
                        float wishbili = (float)_tWidth / (float)_tHeight;
                        switch (ThumbStyle)
                        {
                            case 1:
                                if (_Width <= _tWidth)
                                ...
                                {
                                    _tHeight = (int)((float)_tWidth / realbili);
                                }
                                break;
                            case 2:
                                if (_Height <= _tHeight)
                                ...
                                {
                                    _tWidth = (int)((float)_tHeight * realbili);
                                }
                                break;
                            case 3:
                                //实际图比缩略图最小尺寸更宽矮，以宽为准
                                ...
                                {
                                    int temp_width = _Width;
                                    _Width = _Height * _tWidth / _tHeight;
                                    y = 0;
                                    x = (temp_width - _Width) / 2;
                                }
                                //实际图比缩略图最小尺寸更高长，以高为准
                                else
                                {
                                    int temp_Height = _Height;
                                    _Height = _Width * _tHeight / _tWidth;
                                    x = 0;
                                    y = (temp_Height - _Height) / 2;
                                }
                                break;
                            default:
                                //实际图比缩略图最大尺寸更宽矮，以宽为准
                    ...
                    this.OutThumbFileName = FName.Split('.').GetValue(0).ToString() + "_t." + FName.Split('.').GetValue(1).ToString();
                    string ImgFilePath = _SavePath + this.OutThumbFileName;

                    //System.Drawing.Image newImg = Img.GetThumbnailImage(_tWidth, _tHeight, null, System.IntPtr.Zero);
                    //newImg.Save(ImgFilePath);
                    //newImg.Dispose();

                    System.Drawing.Image tImg = System.Drawing.Image.FromStream(newFile);
                    //接着创建一个System.Drawing.Bitmap对象，并设置你希望的缩略图的宽度和高度。
                    Bitmap bmp = new Bitmap(_tWidth, _tHeight);
                    ...
                    //把原始图像绘制成上面所设置宽高的缩小图
                    System.Drawing.Rectangle rectDestination = new System.Drawing.Rectangle(0, 0, _tWidth, _tHeight);
                    gr.DrawImage(tImg, rectDestination, x, y, _Width, _Height, GraphicsUnit.Pixel);
                    //保存图像
                ...
                if (_IsCreateMidImg)
                {
                    ...
                    if (_Width <= _mWidth && _Height <= _mHeight)
                    {
                        _mWidth = _Width;
                        _mHeight = _Height;
                    }
                    ...
                    {
                        float realbili = (float)_Width / (float)_Height;
                        float wishbili = (float)_mWidth / (float)_mHeight;
                        switch (MidStyle)
                        {
                            case 1:
                                if (_Width <= _mWidth)
                                ...
                                {
                                    _mHeight = (int)((float)_mWidth / realbili);
                                }
                                break;
                            case 2:
                                if (_Height <= _mHeight)
                                ...
                                {
                                    _mWidth = (int)((float)_mHeight * realbili);
                                }
                                break;
                            case 3:
                                //实际图比缩略图最小尺寸更宽矮，以宽为准
                                ...
                                {
                                    int temp_width = _Width;
                                    _Width = _Height * _mWidth / _mHeight;
                                    y = 0;
                                    x = (temp_width - _Width) / 2;
                                }
                                //实际图比缩略图最小尺寸更高长，以高为准
                                ...
                                {
                                    int temp_Height = _Height;
                                    _Height = _Width * _mHeight / _mWidth;
                                    x = 0;
                                    y = (temp_Height - _Height) / 2;
                                }
                                break;
                    ...
                    this.OutMidFileName = FName.Split('.').GetValue(0).ToString() + "_m." + FName.Split('.').GetValue(1).ToString();
                    string ImgFilePath = _SavePath + this.OutMidFileName;

                    //System.Drawing.Image newImg = Img.GetThumbnailImage(_mWidth, _mHeight, null, System.IntPtr.Zero);
                    //newImg.Save(ImgFilePath);
                    //newImg.Dispose();

                    System.Drawing.Image mImg = System.Drawing.Image.FromStream(newFile);
                    //接着创建一个System.Drawing.Bitmap对象，并设置你希望的缩略图的宽度和高度。
                    Bitmap bmp = new Bitmap(_mWidth, _mHeight);
                    //从Bitmap创建一个System.Drawing.Graphics对象，用来绘制高质量的缩小图。
                    System.Drawing.Graphics gr = System.Drawing.Graphics.FromImage(bmp);
                    //设置 System.Drawing.Graphics对象的SmoothingMode属性为HighQuality
                    gr.SmoothingMode = System.Drawing.Drawing2D.SmoothingMode.HighQuality;
                    ...
                    gr.InterpolationMode = System.Drawing.Drawing2D.InterpolationMode.High;
                    //把原始图像绘制成上面所设置宽高的缩小图
                    System.Drawing.Rectangle rectDestination = new System.Drawing.Rectangle(0, 0, _mWidth, _mHeight);
                    gr.DrawImage(mImg, rectDestination, x, y, _Width, _Height, GraphicsUnit.Pixel);
                    //保存图像
                    bmp.Save(ImgFilePath);
                    //释放资源
                    bmp.Dispose();
